...
//            return repository.findById(itemId).orElseThrow();
    ...
    public CommentDto addComment(Comment comment, Long itemId, Long userId) {
        if(userRepository.existsById(userId) & repository.existsById(itemId)) {
            ...
            if(!comment.getText().isBlank() & booking.getStatus().equals(BookingState.APPROVED)) {
...
//        List<Booking> bookingList = bookingRepository.findByItemOrderByEndDesc(itemId);
        ...
        if(item.getOwner().equals(userId)) {
            if(bookingList.isEmpty()) {
                ...
                BookingDate bookingLast = new BookingDate(bookingList.get(0).getId(), bookingList.get(0).getBooker());
                BookingDate bookingNext = new BookingDate(bookingList.get(1).getId(), bookingList.get(1).getBooker());
